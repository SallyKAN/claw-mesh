...
"$BINARY" up --port "$COORD_PORT" --token "$TOKEN" --allow-private --data-dir "$TMPDIR_TEST/data" --config "$TMPDIR_TEST/claw-mesh.yaml" >"$COORD_LOG" 2>&1 &
COORD_PID=$!
...
"$BINARY" up --port "$COORD_PORT2" --token "$TOKEN" --allow-private --data-dir "$TMPDIR_TEST/data" --config "$TMPDIR_TEST/claw-mesh.yaml" >"$COORD_LOG2" 2>&1 &
COORD_PID=$!
